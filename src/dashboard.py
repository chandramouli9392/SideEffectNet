# dashboard.py (FINAL – Full app with Lottie JSON background animation)
...
def load_data():
    if not EDGE_CSV.exists() or not RISK_CSV.exists():
        return (
            pd.DataFrame(columns=["drug_name", "side_effect", "freq_pct"]),
            pd.DataFrame(columns=["drug_name", "risk_score"]),
        )
...
side_effect_lookup = {
    drug: list(group["side_effect"]) for drug, group in edges_df.groupby("drug_name")
}
...
lottie_bg = load_lottie_json("background_animation.json")
...
with st.sidebar:
    ...
    reduce_motion = st.checkbox(
        ...
        value=False,
        help="Disable background animation for accessibility",
    )

# -----------------------
# Simple glass card styling (no crazy background now)
# -----------------------
st.markdown(
    """
<style>
.stMarkdown, .stFrame, .stDataFrameContainer, .stDataFrame,
.stAlert, .stMetric, .stButton {
...
with st.sidebar:
    ...
    if risk_df.empty:
        ...
        min_risk, max_risk = (
            float(risk_df["risk_score"].min()),
            float(risk_df["risk_score"].max()),
        )
...
# -----------------------
# Main app content (title + Lottie hero)
# -----------------------
st.title("SideEffectNet: Drug Safety Analytics")
st.markdown(
    "Explore drug-side effect relationships, risk scores, and polypharmacy risks."
)

# Lottie hero (show only if loaded and animation not reduced)
if lottie_bg and not reduce_motion:
    st_lottie(
        lottie_bg,
        height=260,
        key="hero_animation",
...
with tab1:
    ...
    with col1:
        ...
        drug = st.selectbox(
            "Search for a drug:", options=[""] + drug_list, index=0, key="tab1_drug_search"
        )
        ...
        if drug:
            if drug not in risk_map:
                ...
                score = risk_map[drug]
                risk_color = (
                    "red" if score > 0.7 else "orange" if score > 0.4 else "green"
                )
                st.markdown(
                    f"""
<div style="border-radius: 0.5rem; padding: 1rem;
            background-color: rgba(255,255,255,0.85);
            border-left: 0.3rem solid {risk_color}">
  <h3 style="margin-top: 0; color: #24292f;">{drug}</h3>
  <div style="font-size: 2rem; font-weight: bold; color: {risk_color};">
    {score:.3f}
  </div>
...
""",
    ...
    with col2:
        if drug and drug in side_effect_lookup and st.checkbox(
            ...
            if se_list:
                ...
                for se in se_list[:20]:
                    sg.add_node(
                        se, color="#EF553B", size=20, title=f"Side Effect: {se}"
                    )
                    if G.has_edge(drug, se):
                        ...
                        sg.add_edge(
                            drug,
                            se,
                            value=freq if isinstance(freq, (int, float)) else 1,
        ...
        with st.spinner("Analyzing alternatives..."):
            ...
            if suggestions:
                sugg_df = (
                    ...
                    .sort_values(["Shared Effects", "Risk Reduction"],
...
with tab3:
    ...
    if risk_df.empty:
        ...
        filtered = pd.DataFrame(columns=["drug_name", "risk_score"])
    else:
        filtered = risk_df[
    ...
    with col1:
        drugs_in_range_color = (
            "green" if len(filtered) > 50 else "orange" if len(filtered) > 20 else "red"
        )
        st.markdown(
            f"""
<div style="border-radius: 0.5rem; padding: 1rem;
            background-color: rgba(255,255,255,0.85);
            border-left: 0.3rem solid {drugs_in_range_color};
            margin-bottom: 1rem;">
  <div style="font-size: 1rem; color: #57606a;">Drugs in Range</div>
  <div style="font-size: 1.5rem; font-weight: bold;
              color: {drugs_in_range_color};">
    {len(filtered)}
  </div>
</div>
""",
        ...
        if not filtered.empty:
            ...
            avg_risk_color = (
                "green" if avg_risk <= 0.4 else "orange" if avg_risk <= 0.7 else "red"
            )
            st.markdown(
                f"""
<div style="border-radius: 0.5rem; padding: 1rem;
            background-color: rgba(255,255,255,0.85);
            border-left: 0.3rem solid {avg_risk_color};
            margin-bottom: 1rem;">
  <div style="font-size: 1rem; color: #57606a;">Average Risk</div>
  <div style="font-size: 1.5rem; font-weight: bold;
              color: {avg_risk_color};">
    {avg_risk:.3f}
  </div>
</div>
""",
...
<div style="display: flex; align-items: center; margin-bottom: 0.5rem;">
  <div style="width: 20px; height: 20px; background-color: orange;
              margin-right: 10px; border-radius: 50%;"></div>
    ...
    if len(selected_drugs) >= 2:
        ...
        with col1:
            ...
            for d in selected_drugs:
                ...
                combined_score += risk_map.get(d, 0.0)

            avg_score = combined_score / len(selected_drugs)
            max_score = max(risk_map.get(d, 0.0) for d in selected_drugs)

            st.markdown(
                f"""
<div style="border-radius: 0.5rem; padding: 1rem;
            background-color: rgba(255,255,255,0.85);
            border-left: 0.3rem solid blue; margin-bottom: 1rem;">
  <div style="font-size: 1rem; color: #57606a;">Average Risk</div>
  <div style="font-size: 1.5rem; font-weight: bold; color: blue;">
    {avg_score:.3f}
  </div>
</div>
""",
...
<div style="border-radius: 0.5rem; padding: 1rem;
            background-color: rgba(255,255,255,0.85);
            border-left: 0.3rem solid red; margin-bottom: 1rem;">
  <div style="font-size: 1rem; color: #57606a;">Highest Individual Risk</div>
  <div style="font-size: 1.5rem; font-weight: bold; color: red;">
    {max_score:.3f}
  </div>
</div>
""",
...
<div style="border-radius: 0.5rem; padding: 1rem;
            background-color: rgba(255,255,255,0.85);
            border-left: 0.3rem solid green; margin-bottom: 1rem;">
...
<div style="border-radius: 0.5rem; padding: 1rem;
            background-color: rgba(255,255,255,0.85);
            border-left: 0.3rem solid orange; margin-bottom: 1rem;">
  <div style="font-size: 1rem; color: #57606a;">Overlapping Side Effects</div>
  <div style="font-size: 1.5rem; font-weight: bold; color: orange;">
    {len(overlap_effects) if overlap_effects else 0}
  </div>
</div>
""",
                unsafe_allow_html=True,
        ...
        with col3:
            risk_level = (
                "High" if avg_score > 0.7 else "Medium" if avg_score > 0.4 else "Low"
            )
...
<div style="border-radius: 0.5rem; padding: 1rem;
            background-color: rgba(255,255,255,0.85);
            border-left: 0.3rem solid {color}; margin-bottom: 1rem;">
...
""",
        ...
        tab_overlap, tab_radar = st.tabs(["Side Effect Overlap", "Risk Comparison"])

        with tab_overlap:
            if overlap_effects:
                st.markdown("### Overlapping Side Effects")
                for effect in list(overlap_effects)[:20]:
...
""",
    ...
    if drug_a and drug_b:
        ...
        risk_a = risk_map.get(drug_a, 0.0)
        risk_b = risk_map.get(drug_b, 0.0)
        ...
        with metrics_col1:
            risk_a_color = (
                "red" if risk_a > 0.7 else "orange" if risk_a > 0.4 else "green"
            )
            st.markdown(
                f"""
<div style="border-radius: 0.5rem; padding: 1rem;
            background-color: rgba(255,255,255,0.85);
            border-left: 0.3rem solid {risk_a_color};
            margin-bottom: 1rem;">
  <div style="font-size: 1rem; color: #57606a;">{drug_a} Risk Score</div>
  <div style="font-size: 1.5rem; font-weight: bold; color: {risk_a_color};">
    {risk_a:.3f}
  </div>
</div>
""",
        ...
        with metrics_col2:
            risk_b_color = (
                "red" if risk_b > 0.7 else "orange" if risk_b > 0.4 else "green"
            )
            st.markdown(
                f"""
<div style="border-radius: 0.5rem; padding: 1rem;
            background-color: rgba(255,255,255,0.85);
            border-left: 0.3rem solid {risk_b_color};
            margin-bottom: 1rem;">
  <div style="font-size: 1rem; color: #57606a;">{drug_b} Risk Score</div>
  <div style="font-size: 1.5rem; font-weight: bold; color: {risk_b_color};">
    {risk_b:.3f}
  </div>
</div>
""",
...
<div style="border-radius: 0.5rem; padding: 1rem;
            background-color: rgba(255,255,255,0.85);
            border-left: 0.3rem solid {overlap_color};
            margin-bottom: 1rem;">
  <div style="font-size: 1rem; color: #57606a;">Shared Side Effects</div>
  <div style="font-size: 1.5rem; font-weight: bold; color: {overlap_color};">
    {len(overlapping)}
  </div>
</div>
""",
